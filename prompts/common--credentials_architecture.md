...
### CloudFormation Writer Requirements:
  - The CloudFormation templates **must** reference Secrets Manager ARNs for all secrets.
  - No plaintext secrets or passwords should appear in the templates.
  - The database name must be constructed dynamically using the stack identifier and a suffix, not passed as a parameter.
