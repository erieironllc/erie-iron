...
- OpenTofu deployment or rollback, including stack rotation and parameter validation.
...
- **never** add OpenTofu parameters named `DBName` or `DBPassword`.
...
- **never** attempt to compute database connection details in non-Django code by hand. All non-Django runtimes must call `get_pg8000_connection()` from `erieiron_public.agent_tools` and follow:
  ```python
  with get_pg8000_connection() as conn:
      conn.cursor().execute(<sql>)
  ```
  No alternative helper or manual assembly is allowed.
