...
- For any non-Django code paths under test (Lambda functions, CLI tools, background workers) that require database connectivity, ensure they open connections with `get_pg8000_connection()` from `erieiron_public.agent_tools` via:
  ```python
  with get_pg8000_connection() as conn:
      conn.cursor().execute(<sql>)
  ```
  Do not approve or introduce alternate database discovery or connection mechanisms.
