...
- OpenTofu stacks (`opentofu/foundation/stack.tf` and/or `opentofu/application/stack.tf`) must provision the SES receipt rule set and rules needed for inbound email, using names derived from `!Ref StackIdentifier` and `!Ref DomainName`.
